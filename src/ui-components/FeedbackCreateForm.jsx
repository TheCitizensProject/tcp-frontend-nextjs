...
export default function FeedbackCreateForm(props) {
  ...
  const resetStateValues = () => {
    ...
    setBusinessConsumers(initialValues.businessConsumers);
    setPublicSafety(initialValues.publicSafety);
    setTransportation(initialValues.transportation);
    setCultureRecreation(initialValues.cultureRecreation);
    setTrashRecycling(initialValues.trashRecycling);
    setEnvironment(initialValues.environment);
    setSidewalksStreetsHighways(initialValues.sidewalksStreetsHighways);
    setNoise(initialValues.noise);
    setErrors({});
  };
  const validations = {
    emailID: [],
